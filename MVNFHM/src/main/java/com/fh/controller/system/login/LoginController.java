...
public class LoginController extends BaseController {
	...
	public ModelAndView identity()throws Exception{
		ModelAndView mv = this.getModelAndView();
		/*PageData pd = new PageData();
		pd = this.getPageData();
		pd = this.setLoginPd(pd);*/	//设置登录页面的配置参数
		mv.setViewName("identity");
		/*mv.addObject("pd",pd);*/
		return mv;
	}
			...
			if(null == code || "".equals(code)){//判断效验码
				...
				if(Tools.notEmpty(sessionCode) && sessionCode.equalsIgnoreCase(code)){		//判断登录验证码
					...
					if(pd != null){
						...
						user.setSTORE(pd.getString("STORE"));
						user.setCITY(pd.getString("CITY"));
	...
	public ModelAndView login_index(@PathVariable("changeMenu") String changeMenu){
		...
		try{
			...
			if (user != null) {
				...
				String ROLE_IDS = user.getROLE_IDS();//角色副职id
