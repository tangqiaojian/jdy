...
function getHeadMsg(){
    $.ajax({
        ...
        success: function(data){
            var dt = new Date();
            var time = dt.getFullYear() + "年";
            time += (dt.getMonth() + 1) + "月";
            time += dt.getDate() + "日";
            $.each(data.list, function(i, list){
                if (list.USER_ID!=null&&list.USER_ID!=""){
                    $("#user_info").html(time+'&nbsp;&nbsp;你好！&nbsp;&nbsp;'+list.NAME);//登陆者资料
                }else {
                    $("#user_info").html("已有账号，马上"+"<a href= 'login_toLogin' style='color: white'>登录</a>");//登陆者资料
                }
                user = list.USERNAME;
