...
<body class="no-skin">
						...
						<table style="margin-top:5px;">
							<tr>
								...
								<td style="vertical-align:top;padding-left:2px;">
								 	<select class="chosen-select form-control" name="order_type" id="order_type" data-placeholder="请选择订单状态" style="vertical-align:top;width: 120px;">
									   <option value=""></option>
									   <option value="">全部</option>
									   <option value="0" ${pd.order_type_select=='0'?"selected":"" }>待揽收</option>
									   <option value="1" ${pd.order_type_select=='1'?"selected":"" }>已揽收</option>
									   <option value="2" ${pd.order_type_select=='2'?"selected":"" }>已入库</option>
									   <option value="3" ${pd.order_type_select=='3'?"selected":"" }>清关中</option>
									   <option value="4" ${pd.order_type_select=='4'?"selected":"" }>已签收</option>
								  	</select>
								</td>
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'>
											  <c:choose>
											     <c:when test="${var.STATEA==0}">
											              待揽收
											     </c:when>
											     <c:when test="${var.STATEA==1 || var.STATEA==2 }">
											              已揽收
											     </c:when>
											     <c:when test="${var.STATEA==3 }">
											               已入库
											     </c:when>
											     <c:when test="${var.STATEA==4}">
											              清关中
											     </c:when>
											     <c:when test="${var.STATEA==5}">
											               已签收
											     </c:when>
											  </c:choose>
											</td>
													...
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															...
															<c:choose>
													            <c:when test="${var.STATEA=='0'}">
													              <li>
																    <a style="cursor:pointer;"  onclick="details('${var.ORDER_ID}')"  class="tooltip-success" data-rel="tooltip" title="查看">
																	 <span class="green">
																		 查看
																	   </span>
																     </a>
	...
	<script type="text/javascript">
		...
		function details(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="";
			 diag.URL = '<%=basePath%>order/order_1_details.do?ORDER_ID='+Id;
			 diag.Width = 1080;
			 diag.Height = 600;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
