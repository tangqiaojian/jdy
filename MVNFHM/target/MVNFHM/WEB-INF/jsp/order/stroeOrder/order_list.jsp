                                ...
                                <tr>
                                    <td>
										...
										<span class="input-icon">
											<input type="text" placeholder="这里输入订单编号" class="nav-search-input"
                                                            ...
                                                            <c:choose>
                                                                <c:when test="${var.STATEA=='0'}">
                                                                    <a class="btn btn-xs btn-success" title="查看"
                                                                       onclick="details"
                                                                       href="<%=basePath%>storeOrder/order_store_details_0.do?ORDER_ID=${var.ORDER_ID}">
                                                                        查看
                                                                    </a>
                                                                </c:when>
                                                                <c:otherwise>
                                                                    <a class="btn btn-xs btn-success" title="查看"
                                                                       onclick="details('${var.ORDER_ID}')">
                                                                        查看
                                                                    </a>
                                                                </c:otherwise>
                                                            </c:choose>

                                                            <a class="btn btn-xs btn-error" title="删除"
                                                                    ...
                                                                    <c:choose>
																	 ...
																	 <span class="green">
                                                                        ...
                                                                        <c:otherwise>
                                                                            <li>
                                                                                <a style="cursor:pointer;"
                                                                                   href="<%=basePath%>order/order_1_details.do?ORDER_ID=${var.ORDER_ID}"
                                                                                   class="tooltip-success"
                                                                                   data-rel="tooltip" title="修改">
																	 <span class="green">
																		 查看
                            ...
                            <table id="table_report_3" class="table table-striped table-bordered table-hover">
                                <tr>
                                    <td style="text-align: center;" colspan="10">
                                        <a class="btn btn-mini btn-primary" onclick="makeAll('确定发往总仓?')">发往总仓</a>
                                    </td>
                                </tr>
                            </table>
                        </c:if>
...
<script type="text/javascript">
    ...
    function details(Id) {
        ...
        diag.Drag = true;
        diag.Title = "";
        diag.URL = '<%=basePath%>order/order_1_details.do?dy=' + '${pd.dy}' + '&ORDER_ID=' + Id;
        diag.Width = 1000;
        diag.Height = 800;
            ...
            if (result) {
                ...
                if (str == '') {
                    ...
                    if (msg == '确定发往总仓?') {
                        ...
                        $.ajax({
                            type: "POST",
                            url: '<%=basePath%>storeOrder/updateMakeState.do?tm=' + new Date().getTime(),
                            data: {DATA_IDS: str},
                            dataType: 'json',
