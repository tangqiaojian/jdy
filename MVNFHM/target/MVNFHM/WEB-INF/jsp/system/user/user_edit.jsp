									...
									<table id="table_report" class="table table-striped table-bordered table-hover">
										...
										<tr>
											...
											<c:forEach items="${roleList}" var="role">
												<option value="${role.ROLE_ID }" ${role.ROLE_ID == pd.ROLE_ID?"selected":"" }>${role.ROLE_NAME }</option>
											</c:forEach>
											</select>
											</td>
										</tr>
										...
										<tr>
											...
											<div>
												<select multiple="" class="chosen-select form-control" id="form-field-select-4" data-placeholder="选择副职角色">
													<c:forEach items="${roleList}" var="role">
														<option onclick="setROLE_IDS('${role.ROLE_ID }')" value="${role.ROLE_ID }" ${role.RIGHTS == '1'?"selected":"" }>${role.ROLE_NAME }</option>
													</c:forEach>
												</select>
											</div>
											</td>
										...
										<tr class="store">
											...
											<td>
											<div>
												<select name="STORE" id="STORE" class="chosen-select form-control" id="form-field-select-4" data-placeholder="选择门店">
													<option value=""></option>
													<c:forEach items="${pd.stores}" var="s">
														<option value="${s.STORE_ID }" ${pd.STORE == s.STORE_ID?"selected":"" }>${s.S_NAME }</option>
													</c:forEach>
												</select>
											</div>
											</td>
...
<script type="text/javascript">
	...
	//a809f300d155452680a8760ec8592408
	//选择门店主键
	$(function(){
	  var store_role='a809f300d155452680a8760ec8592408';
